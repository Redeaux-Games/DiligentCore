    ...
    TTextureBase{pRefCounters, TexViewObjAllocator, pRenderDeviceD3D12, TexDesc}
{
    ...
    if (m_Desc.Usage == USAGE_IMMUTABLE || m_Desc.Usage == USAGE_DEFAULT || m_Desc.Usage == USAGE_DYNAMIC)
    {
        VERIFY(m_Desc.Usage != USAGE_DYNAMIC || PlatformMisc::CountOneBits(m_Desc.CommandQueueMask) <= 1,
               "CommandQueueMask must contain one bit, error must be handled in ValidateTextureDesc()");
        ...
        auto D3D12State = ResourceStateFlagsToD3D12ResourceStates(InitialState) & StateMask;
        auto hr =
