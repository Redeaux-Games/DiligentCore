    ...
    while (States != 0)
    {
        ...
        switch (State)
        {
            ...
            case RESOURCE_STATE_COPY_SOURCE:
                ...
                {
                    Result = false;
                    CHECK_PARAMETER(Name, " contains state ", GetResourceStateFlagString(State), " which is not supported in ", GetContextTypeString(ContextType), " context");
                }
                break;
...
#define CHECK_STATE_TRANSITION_DESC(Expr, ...) CHECK_PARAMETER(Expr, "State transition parameters are invalid: ", __VA_ARGS__)
    ...
    CHECK_STATE_TRANSITION_DESC((CommandQueueMask & (Uint64{1} << CmdQueueInd)) != 0,
