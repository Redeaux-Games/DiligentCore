...
CommandQueueVkImpl::~CommandQueueVkImpl()
{
    ...
    if (m_pFence)
        m_pFence->ImmediatellyReleaseResources();
...
Uint64 CommandQueueVkImpl::Submit(const VkSubmitInfo& InSubmitInfo)
{
    ...
    if (m_UseTimelineSemaphore)
    {
        ...
        // Semaphore signal operations that are defined by vkQueueSubmit additionally include all commands that occur earlier in submission order.
